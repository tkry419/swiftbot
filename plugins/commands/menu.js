...
export default {
  ...
  async run({ replyImg, reply, settings, box, font, t, args, isOwner, sender }) {
    ...
    // 2. GROUP KWA CATEGORY
    const categories = {}
    for (const cmd of allCommands) {
      if (cmd.owner &&!isOwner &&!settings.sudos.includes(sender)) continue
    ...
    if (args[0]) {
      ...
      return await replyImg(botPic, translated)
    }
    ...
    const menuText = box('menu', {
      ...
      prefix: prefix? font(prefix, 'monospace') : font('None', 'italic'),
      mode: settings.publicMode? 'Public' : 'Private',
      categories: categoryItems,
      total: totalCmds,
      usage: font(`Type "${prefix || ''}menu <number>"`, 'italic'),
      example: font(`Example: "${prefix || ''}menu 1"`, 'italic')
    })

    const translated = await t(menuText)
    await replyImg(botPic, translated)
  }
}